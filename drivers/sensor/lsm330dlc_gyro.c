	...
	(struct lsm330dlc_gyro_data *data)
{
	...
	input_report_rel(data->input_dev, REL_MISC, temp);
...
static ssize_t lsm330dlc_gyro_show_sensor_value(struct device *dev,
                                struct device_attribute *attr, char *buf)
{
    struct lsm330dlc_gyro_data *data = dev_get_drvdata(dev);
    int res, i, j;
    ...
    if (temp < 0) {
        ...
        return sprintf(buf, "%d,%d,%d,%d\n",
                gyro_adjusted[0], gyro_adjusted[1], gyro_adjusted[2], temp);
}
...
static DEVICE_ATTR(sensor_value, 0444,
		        lsm330dlc_gyro_show_sensor_value, NULL);
			       ...
			       const struct i2c_device_id *devid)
{
	...
	if (device_create_file(&input_dev->dev,
        ...
        if (device_create_file(&input_dev->dev, &dev_attr_sensor_value) < 0) {
                pr_err("%s: Failed to create device file(%s)!\n", __func__,
...
err_device_create_ex:
        device_remove_file(&input_dev->dev, &dev_attr_sensor_value);
err_device_create:
...
static int lsm330dlc_gyro_remove(struct i2c_client *client)
{
	...
	device_remove_file(&data->input_dev->dev, &dev_attr_sensor_value);
	input_unregister_device(data->input_dev);
	mutex_destroy(&data->lock);
	kfree(data);
